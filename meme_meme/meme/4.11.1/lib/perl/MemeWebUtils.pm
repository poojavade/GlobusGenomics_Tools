...
use lib qw(/mnt/galaxyTools/tools/meme/lib/perl);
...
unless ($@) {
  Log::Log4perl::init('/mnt/galaxyTools/tools/meme/etc/logging.conf');
  $logger = Log::Log4perl->get_logger('meme.service.utils');
}

my $service_invocation_log_dir = '/mnt/galaxyTools/tools/meme/LOGS';
my $tmpdir = '';
...
sub update_status {
  ...
  my $template = HTML::Template->new(filename => '/mnt/galaxyTools/tools/meme/etc/job_status.tmpl');
